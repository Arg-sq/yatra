...
import { renderLocation, responseErrorHandler } from "@/services/helper";
...
const Approval: React.FC = () => {
  ...
  const { data, error, mutate } = useSWR(
    `/admin/hotel/hotel-application?page=${page}`
  );
      ...
      {loading ? (
        ...
        >
          ...
          <Column
            ...
            dataIndex="address"
            key="address"
            render={(_, { city, state, country }: DataType) =>
              renderLocation({ city, state, country })
            }
          />
          <Column
            title="Description"
          ...
          <Column
            ...
            render={(_: any, record: DataType) => (
              <Space size="middle">
                {record.status === 0 ? (
                  <Tag className="bg-warning rounded px-3">Pending</Tag>
                ) : (
                  <Tag className="bg-success text-white rounded px-3">
                    Completed
                  </Tag>
                )}
              </Space>
                ...
                {record.status === 0 ? (
                  <>
                    <Button
                      ...
                      onClick={() =>
                        router.push({
                          pathname: "/superadmin/hotels/create",
                          query: {
                            name: record.name,
                            email: record.email,
                    ...
                    <Popconfirm
                      title="Are you sure to complete this application?"
                      onConfirm={() => completeApplicationHandler(record.id)}
                    >
